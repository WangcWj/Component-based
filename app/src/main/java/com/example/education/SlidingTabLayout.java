...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.item_sliding_tab_layout, null);
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        if (-1 != mContainerGravity) {
            mTabsContainer.setGravity(mContainerGravity);
        }
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            TextView mTvTabTitle = v.findViewById(R.id.tv_tab_title);
            if (mTvTabTitle != null) {
                mTvTabTitle.setTextColor(i == mCurrentTab ? mTextSelectColor : mTextUnselectColor);
                mTvTabTitle.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextsize);
                mTvTabTitle.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
                if (mTextAllCaps) {
                    mTvTabTitle.setText(mTvTabTitle.getText().toString().toUpperCase());
                }

                if (mTextBold == TEXT_BOLD_BOTH) {
                    mTvTabTitle.getPaint().setFakeBoldText(true);
                } else if (mTextBold == TEXT_BOLD_NONE) {
                    mTvTabTitle.getPaint().setFakeBoldText(false);
                }
            }
        }
    }
    ...
    private void calcIndicatorRect() {
        ...
        if (this.mCurrentTab < mTabCount - 1) {
            ...
            if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
                ...
                mTextPaint.setTextSize(mTextsize);
                float nextTextWidth = mTextPaint.measureText(mNextTabTitle.getText().toString());
    ...
    public TextView getTitleView(int tab) {
        ...
        TextView mTvTabTitle = (TextView) tabView.findViewById(R.id.tv_tab_title);
        return mTvTabTitle;
    }
