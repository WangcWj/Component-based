...
public class PtrFrameLayout extends ViewGroup {
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                int pointerCount1 = e.getPointerCount();
                if (pointerCount1 == 2) {
                    return dispatchTouchEventSupper(e);
                }
                float x = e.getX();
                float y = e.getY();
                float distance = Math.abs(x - startMoveX);
                float distanceY = Math.abs(y - startMoveY);
                if (distance > 50f && distanceY < 50f) {
                    return dispatchTouchEventSupper(e);
                }
                mLastMoveEvent = e;
    ...
    final public void refreshComplete() {
        ...
        if (delay <= 0) {
            ...
            postDelayed(new PostDelayRunnable(this), delay);
            if (DEBUG) {
    ...
    public ILoadMoreViewFactory.ILoadMoreView mLoadMoreView;
    ...
    public void setLoadMoreEnable(boolean loadMoreEnable) {
        ...
        if (!hasInitLoadMoreView && isLoadMoreEnable) {
            ...
            if (null == loadMoreViewFactory) {
                loadMoreViewFactory = getLoadMoreFactory();
            }
            mLoadMoreView = loadMoreViewFactory.madeLoadMoreView();
    ...
    @NonNull
    protected ILoadMoreViewFactory getLoadMoreFactory() {
        return new DefaultLoadMoreViewFooter();
    }
    ...
    private static class PostDelayRunnable implements Runnable {
        ...
        private final SoftReference<PtrFrameLayout> mSoftReference;

        public PostDelayRunnable(PtrFrameLayout ptrFrameLayout) {
            mSoftReference = new SoftReference<>(ptrFrameLayout);
        }

        @Override
        public void run() {
            PtrFrameLayout ptrFrameLayout = mSoftReference.get();
